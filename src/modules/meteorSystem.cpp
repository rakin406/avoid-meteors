...
void createMeteor(flecs::world& world) {
    ...
    const float randomScale{tools::getRandomValue<float>(MeteorSystem::MIN_SCALE, MeteorSystem::MAX_SCALE)};
...
modules::MeteorSystem::MeteorSystem(flecs::world& world) {
    ...
    world.system<RenderWindow, Angle, Sprite, Transform, Meteor>("LoadMeteors")
        ...
        .singleton()
        .each(
            [](flecs::entity meteor, RenderWindow& window, Angle& angle, Sprite& sprite, Transform& transform, Meteor) {
                // Load meteor sprite
                ...
                float randomPosX{getRandomValue<float>(0.0f, static_cast<float>(WINDOW_WIDTH) - textureSize.x)};
...

                {
                    ...
                    float middlePosX{static_cast<float>(WINDOW_WIDTH / 2) - ((textureSize.x * transform.scale.x) / 2)};
                    ...
                    // Set random angle
                    angle.value = tools::getRandomValue<float>(0.0f, std::atan(opposite / adjacent));
                }
            });
    ...
    world.system<Transform, const Angle, const Velocity, Meteor>("Move")
        .kind(flecs::OnUpdate)
        .each([](flecs::iter& it, size_t index, Transform& transform, const Angle& angle, const Velocity& velocity,
                 Meteor) {
            auto meteor{it.entity(index)};

            // Move meteor based on direction
            if (meteor.has(Direction::Left)) {
                transform.position.x -= (velocity.x * std::cos(angle.value)) * it.delta_time();
            } else if (meteor.has(Direction::Right)) {
                transform.position.x += (velocity.x * std::cos(angle.value)) * it.delta_time();
            }

            transform.position.y += (velocity.y * std::sin(angle.value)) * it.delta_time();
        });
    ...
    world.system<const Transform, CollisionLayer::Meteor>("MeteorCollision")
        ...
        .each([](flecs::entity meteor, const Transform& transform, CollisionLayer::Meteor) {
